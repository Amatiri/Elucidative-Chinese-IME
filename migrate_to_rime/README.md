...
### 2.11 多字逐字选择（P4-A，2026-09-11 落地并实机复测通过）

**背景**：RIME 的分段**只看字符类**（`abc_segmentor` 读 `speller/alphabet`，见
`abc_segmentor.cc:22`），不知道解书的自动拆分。于是「选完第一个字后的剩余串」会被当成
**一个段**交给查询层，走多段分支只产出「首选字链」。离线实测（真 lua + 真码表）：

| 输入 | 选中第 1 字后剩余段 | 改动前候选 |
| -------------------------- | ---------- | ------------ |
| `bu44ba13`（2 字）            | `ba13`     | 八 / 捌.（2 条，逐字天然保住） |
| `bu44ba13bu44`（3 字）        | `ba13bu44` | 「八不」链（**1 条，粒度丢失**） |
...
（`engine.cc:259-282`）`Forward()` + 重新 `Compose()`，剩余部分自动成为下一段 ——
于是「手动 Left 定位一次 + 连续 Shift+1~5」即可逐字选完；末段到达输入末尾时 `kConfirmed`
配合 ExpressEditor 的 `_auto_commit=true`（`editor.cc:205`）直接上屏整串。
**不需要** ime.py 的「余码补回输入串 / `resolved_chars` / 跳下一段」—— 那套是「状态寄生在
输入串里」的实现手段（`ime.py:489-496`，它的 `main_function` 每次输入变化都重建状态，
见 `ime.py:544-557`）；RIME 用段状态（`status` / `selected_index`）承载同样的语义。

**与前端（`ime.py`）的差异**：

| 项      | ime.py                                     | RIME 侧（2.11）                                  |
| ------ | ------------------------------------------ | -------------------------------------------- |
| 进入逐字   | `=` / `-`（`navigate_parts`）                | `=` 跳到第一个未确认段末尾、`-` 回退重选（2.12）；未按 `=` 时仍可用 Left 手动定位           |
| 选字后    | 余码补回输入串、光标跳末尾、自动跳下一段（`ime.py:488-500`）    | 段被 `partial` 切开、光标跳末尾，剩余段自动重新查询                |
| 已选的字   | 记进 `resolved_chars`，`=`/`-` 跳过，不可回退        | 记在段状态里，**Backspace（`ReopenPreviousSelection`）可退回重选** |
| 输入串    | 被改写（补满满码 + `'` 分隔）                         | 保持用户敲的原样；上屏文本由各段候选拼出（`Composition::GetCommitText`） |

**已知代价 / 边界**：

1. 逐字模式下页号改挂 `comment`（见 2.9 例外段），因为 `preedit` 的 prompt 位条件不再成立；
2. 首个 part 无候选时**退回改动前的整体语义**（不产出候选），不强行跳过该 part ——
   即 ime.py 里「字面段按原码输出」那一层尚未对齐，记为 **P4-A2**；
3. 段内含人工 `'` 时仍走「词语增强预览」（2.8），不被逐字模式接管；
4. 段首可能残留上一位的余码（按 `bu` 选「不44」后段变成 `44ba13…`），查询层照旧跳过这些
   前导字符，但算候选 `end` 时把它们计入偏移（`lead_code_offset`），否则段会被切错位置。

**2026-09-11 键盘复测**：光标左移到拆分点后 Shift+1 —— 选中首字后剩余段自动出下一字的候选，
连续 Shift+1 逐字走完，末字自动上屏；`Backspace` 可退回上一字重选；2 字输入（`bu44ba13`）
的原有流程未受影响。P4-A 闭环。

### 2.12 逐字定位键 `=` / `-`（P4-B，2026-09-11 落地并实机复测通过）

**要解决的问题**：2.11 之后逐字选择本身能连续走了，但**进入**它还得手动按 Left 数步把光标挪到
拆分点（`bu44ba13bu44` 要按 6 次），与 ime.py 的 `=` 手感差一截。

**实现**：新增 `lua_processor@*jieshu_nav`，挂在链首 gate 之后、**key_binder 之前**。
...
**已确认位置怎么取**：`Composition` 的 lua 绑定只暴露 `back()`（`types.cc` 的 CompReg），
而稳定态下最后一段必然紧贴已确认前缀（`Segmentation::Reset` + `Trim`/`Forward`），故
`confirmed_pos`：段已确认 → 取段尾；未确认 → 取段首（= 已确认前缀长度）。

**键位冲突（已核实本机 `default.yaml`，万象）**：`-`/`=` 被绑成 `Page_Up`/`Page_Down`。
nav 排在 key_binder 之前，故 composing 时优先按逐字定位解释 —— **翻页仍用 ↑↓**（2.5 已重绑）。
西文模式（含 `Shift_L` 的 `inline_ascii` 临时态）整体放行，`=`/`-` 原样上屏。

**边界**：已全部确认（无未确认段）时 `=` 吞键不动作；`=`/`-` 在非 composing 时一律放行；
抬键（release）与 `Ctrl`/`Alt`/`Super` 组合一律放行 —— Weasel 会把 release 也送进处理链
（gate 同样显式处理），不放过就会「按一次触发两回」（`-` 会连退两段）；
`jieshu_query_api` 尚未建立时（极端加载顺序）nav 放行不拦，不阻断输入。

**与页码的相互作用**：`=` 把光标收到段中间后，段不再铺到输入末尾，preedit 的 prompt 位失效
——页号自动改挂候选 comment（2.9 例外的第 2、3 种），不会因为用了定位键就丢页码。
